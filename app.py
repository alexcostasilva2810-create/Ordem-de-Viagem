...
# =========================================================
# 1. CONFIGURAÇÃO E DESIGN (RESPIRO NO TOPO)
# =========================================================
...
st.markdown("""
    ...
    /* Estilo para colunas não encavalarem */
...
# =========================================================
# 3. CONEXÃO E CARREGAMENTO (SEM DUPLICATAS)
# =========================================================
def carregar_dados_blindados():
    try:
        ...
        if len(hist_raw) > 1:
            ...
            # Resolve o erro de Duplicate Column Names
...
if st.session_state.pagina_atual == "Capa":
    ...
    # Carrega dados
    ativos, lista_balsas, lista_rotas, df_h = carregar_dados_blindados()

    with st.sidebar:
        st.title("MENU")
        if st.button("🏠 Voltar para Capa", key="btn_voltar_capa"):
            st.session_state.pagina_atual = "Capa"
            st.rerun()
        st.write("---")
        menu = st.radio("Selecione:", ["📊 Simulações", "📜 Histórico"], key="menu_radio")

    if menu == "📊 Simulações":
        st.title("📊 Simulador de Viagem")
        
        # BUSCA (Resolvendo erro de DuplicateWidgetID com chaves únicas)
        with st.expander("🔍 BUSCAR REGISTRO", expanded=False):
            if not df_h.empty:
                id_sel = st.selectbox("Selecione ID:", ["---"] + df_h.iloc[:, 0].tolist(), key="select_busca_id")
                if st.button("CARREGAR DADOS", key="btn_carregar_busca"):
                    st.session_state.dados_edit = df_h[df_h.iloc[:, 0] == id_sel].iloc[0].to_dict()
                    st.rerun()

        d = st.session_state.dados_edit
        v_id = d.get('ID', datetime.now().strftime("VGM-%H%M"))

        # --- GRID 4X3 ---
        l1, l2, l3 = st.columns(3)
        v_emp = l1.selectbox("Empurrador", ativos, index=ativos.index(d['Empurrador']) if d.get('Empurrador') in ativos else 0, key="f_emp")
        try: b_def = ast.literal_eval(d.get('Balsas', '[]')) if '[' in str(d.get('Balsas')) else []
        except: b_def = []
        v_bal = l2.multiselect("Balsas", lista_balsas, default=b_def, key="f_bal")
        v_com = l3.text_input("Comandante", value=d.get('Comandante', ""), key="f_com")

        l4, l5, l6 = st.columns(3)
        oris = sorted(list(set([r[0] for r in lista_rotas if r])))
        dess = sorted(list(set([r[1] for r in lista_rotas if len(r)>1])))
        v_ori = l4.selectbox("Origem", oris, index=oris.index(d['Origem']) if d.get('Origem') in oris else 0, key="f_ori")
        v_des = l5.selectbox("Destino", dess, index=dess.index(d['Destino']) if d.get('Destino') in dess else 0, key="f_des")
        v_chf = l6.text_input("Chefe de Máquinas", value=d.get('Chefe de Máquinas', ""), key="f_chf")

        l7, l8, l9 = st.columns(3)
        v_vol = l7.number_input("Volume (M³)", value=float(str(d.get('Volume',0)).replace('.','').replace(',','.')) if d.get('Volume') else 0.0, key="f_vol")
        v_fat = l8.number_input("Faturamento (R$)", value=float(str(d.get('Faturamento',0)).replace('.','').replace(',','.')) if d.get('Faturamento') else 0.0, key="f_fat")
        v_hor = l9.number_input("Horímetro", value=float(d.get('Horímetro', 0.0)), key="f_hor")

        v_obs = st.text_area("Observações", value=d.get('Observações', ""), key="f_obs")
        
        status = "APROVADO" if v_fat >= 50000 else "ANÁLISE"
        st.write(f"**STATUS:** {status}")

        if st.button("✅ FINALIZAR E GERAR O.S.", key="btn_gerar_pdf"):
            dados_pdf = {
                "ID": v_id, "Empurrador": v_emp, "Rota": f"{v_ori} x {v_des}",
                "Faturamento": f"R$ {v_fat:,.2f}", "Status": status
            }
            pdf_bytes = gerar_pdf_os(dados_pdf)
            st.success("O.S. Gerada!")
            st.download_button("📥 BAIXAR PDF", data=pdf_bytes, file_name=f"OS_{v_id}.pdf", mime="application/pdf", key="btn_download")
